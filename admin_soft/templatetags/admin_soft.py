...
@register.filter
def value_plus_ten(value):
    return int(value) + 5

@register.filter
def value_res_ten(value):
    return int(value) - 5

@register.filter
def get_form(forms: dict, id):
    return forms.get(int(id))

@register.filter
def get_stock(value: dict, id):
    try:
        return value[int(id)].get('stock')
    except:
        return 0